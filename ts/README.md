...
Scope: **stdio parity only** — the same 36 tools, validators, envelope,
...
- `src/tools.ts` — the 36 tools in feature-manifest order, with the deny-list (`DENY_LIST`, `DENIED_FLAGS`) and the fail-closed async receipts (`receipt_submit` / `receipt_status`); `argvEffect` fails typed, legacy `argv` throws the same message; `liveContextEffect` resolves via `ConfigService`.
