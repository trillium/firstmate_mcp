...
TOOLS = {
    ...
    "pr_poll": ("fm-pr-poll.sh", None, False),
    ...
    "test_run": ("fm-test-run.sh", None, True),
...
def generate():
    ...
    lines.append("")
    lines.append("| Area | Mirrored | Denied | Gap | Fork |")
    lines.append("| --- | --- | --- | --- | --- |")
    coverage, _ = coverage_summary_rows()
    for title, mirrored, denied, gap, fork in coverage:
        lines.append(f"| {title} | {mirrored} | {denied} | {gap} | {fork} |")
    lines.append("")
    ...
    lines.append("refusal reason; gap is the explicitly unmirrored port backlog. Fork counts")
    lines.append("trillium/firstmate extensions with no upstream counterpart (spine-classified).")
    return "\n".join(lines) + "\n"
