...
COMMAND_AREAS = {
    ...
    "fm-lab-home.sh": ("sessions", "disposable lab-home minter for gate agents; minting stays out, denied by design"),
    ...
    "fm-groom.sh": ("backlog-decisions", "proactive idea-to-brief-to-dispatch generator, off by default; fork extension, denied by design"),
...
DENY_REASONS = {
    ...
    "lab_home": ("fm-lab-home.sh", "minting a marked lab home lets a gate agent drive fleet lifecycle entrypoints past the gate refusal; only explicitly authorized lab work owns lab homes"),
    ...
    "groom": ("fm-groom.sh", "formulating briefs spends inference budget and dispatching them spawns crewmates; only the owner-run groom path owns launching"),
    ...
    "afk_contract": ("fm-afk-contract.sh", "the away-posture record gates supervision and archives the captain's verbatim mandate clauses; only the /afk entry and return paths own the record"),
    "afk_launch": ("fm-afk-launch.sh", "entry and exit own the away-mode daemon terminal lifecycle and posture record; only the away-mode entry path owns launches"),
    "afk_return": ("fm-afk-return.sh", "the return shuts the daemon down and owns the catch-up gate; only the return path owns returns"),
    "afk_start": ("fm-afk-start.sh", "starting execs the supervise daemon in the foreground; only harness-tracked launch owns daemon starts"),
    "branch_outcome": ("fm-branch-outcome.sh", "the append-only outcome store with its read-cursor and processed-marker protocol is the branch's private channel; only the Pi branch extension and session-start replay own it"),
    "branch_prompt": ("fm-branch-prompt.sh", "the branch system prompt carries the branch's authority context; only the branch host owns prompt construction"),
    "busy_event": ("fm-busy-event.sh", "the sole writer of the semantic busy-state contract; only adapter wiring and the owning spawn own busy writes"),
    "kimi_turnend_hook": ("fm-kimi-turnend-hook.sh", "installing edits $HOME/.kimi-code/config.toml outside any home; only the Kimi session setup owns hook installs"),
    "operational_input": ("fm-operational-input.sh", "encoding mints supervisor-routed marked inputs; only the owning supervision path owns marked-input construction, and parsing stays in-process as a sourced library"),
    "procevent_run": ("fm-procevent.sh", "the runner executes long-polling children to completion as supervised background processes; only the watcher owns event sources"),
    "procevent_lavish": ("fm-procevent-lavish.sh", "the lavish adapter's poll runs a blocking wait on an external review surface; only the process-event runner owns adapter polls"),
    "procevent_quota": ("fm-procevent-quota.sh", "the quota adapter's poll blocks until quota exhausts or errors; only the process-event runner owns adapter polls"),
    "procevent_remote_reply": ("fm-procevent-remote-reply.sh", "reply intake ingests remote status deltas into the parent stream; only the process-event runner owns autohandle"),
    "procevent_when": ("fm-procevent-when.sh", "arming binds and fires an arbitrary action executable on a condition; only the process-event runner owns condition-action sources"),
    "turnend_guard": ("fm-turnend-guard.sh", "the turn-end guard blocks a primary's turn boundary; only the harness Stop hooks fire it"),
    "turnend_guard_cursor": ("fm-turnend-guard-cursor.sh", "the Cursor stop-hook park holds the turn boundary open until a watcher wake; only Cursor's stop step owns the park"),
    "turnend_guard_grok": ("fm-turnend-guard-grok.sh", "the Grok Stop-hook adapter resumes the owning session; only Grok's Stop hook owns it"),
    "wake_grant": ("fm-wake-grant.sh", "grant activation and publication mutate wake-queue ownership under the wake lock; only the wake path owns grants"),
    "watch_checkpoint": ("fm-watch-checkpoint.sh", "a checkpoint runs the watcher in the foreground for up to a bounded window; only the harness checkpoint path owns foreground watcher runs"),
}
