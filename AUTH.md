...
Secondmate/remote verbs carry the same safe-subset discipline: `secondmate_nudge` is notify-only, `secondmate_restart` takes ids only, `secondmate_report` is the note-only form with the destination resolved by the owning helper, `remote_control` is the closed state/route/observe/send subset (no launch, no raw pane access, no remote teardown), `extension_list`/`extension_inspect` read the home-local binding registry only (installing, retiring, handshake-executing, or transferring bindings stays out), and `handoff_move` moves only queued items or resumes pending wakes. Provisioning new secondmate homes stays out (firstmate-owned) unless read-only status.
...
Installs reads stay run-free: `startup_network_report` prints only the deferred stage report (start/run/harvest/wait stay out), `doc_audience_check` validates a home-confined root (arbitrary inventory argv stays out), `home_seed_validate` checks the registry (provisioning stays out), `stow_cascade` enumerates cascade inputs (curation stays with the skill), and `test_isolation_list`/`test_run_list` list test topology (proof and suite runs stay out); bootstraps, check registration, memory-file writes, installs, updates, and lint runs stay firstmate-owned behind the deny-list.
...
Code-forbidden (no tool, refused as unknown): `promote_scout`, `teardown_crew`, `arm_pr_check`, `merge_pr`, `merge_local`, `public_followup_emit`, `relay_link`, `fleet_sync`, `inactive_reconcile`, `backlog_receive`, `session_start`, `sessionstart_run`, `sessionstart_cursor`, `herdr_lab`, `herdr_ci_cleanup`, `session_cleanup`, `claude_trust`, `agy_trust`, `claude_stop_autoarm`, `herdr_eventwait`, `herdr_workspace_move`, `backend_select`, `on_execute`, `config_push`, `remote_entrypoint`, `remote_herdr_guard`, `remote_provision`, `remote_seed`, `inherit_push`, `remote_inherit`, `reap_orphans`, `remote_worker`, `bootstrap`, `check_register`, `check_unregister`, `agents_md_ensure`, `install_actionlint`, `install_herdr`, `install_shellcheck`, `install_treehouse`, `update`, `workflow_lint`.
