...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout, secondmate-charter, and Herdr-lab briefs; load hooks when FM_HOOK_BEADS_ID or other hook env vars are set |
...
| `fm-project-dir-lib.sh`  | Single owner of how a project argument (bare name, `projects/<name>`, or explicit path) names a clone directory; shared by fm-brief.sh, fm-fleet-sync.sh, and fm-spawn.sh so the two halves of a dispatch cannot disagree |
...
| `fm-pool-reclaim.sh`     | Fail-open reclaim of abandoned treehouse pool slots (stale process-free leases and firstmate-only-dirty spent slots); fm-spawn.sh pre-flights it when the pool is exhausted |
...
| `fm-wake-memo.sh`        | Durable log mapping each drained wake identity to its handling outcome, so repeat wakes consult by lookup |
| `fm-classify-lib.sh`     | Shared wake-classification vocabulary and durable keyed-decision folds and scans     |
| `fm-send.sh`             | Send one verified literal line or supported key through the target's backend; `--raw` sends unverified best-effort to a raw/unmanaged pane |
