...
`DENY_LIST` covers code-writing and landing tools (`promote_scout`, `teardown_crew`, `arm_pr_check`, `merge_pr`, `merge_local`), daemon and supervision control (`daemon_start/stop/restart`, `watch_start/stop`), direct repo mutation (`repo_edit/commit/push/merge`), un-gated public relay emission and linking (`public_followup_emit`, `relay_link`), fleet synchronization and inactive outcome reconciliation (`fleet_sync`, `inactive_reconcile`), and remote backlog outbox receipt (`backlog_receive`).
