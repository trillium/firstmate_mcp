...
`DENY_LIST` covers code-writing and landing tools (`promote_scout`, `teardown_crew`, `arm_pr_check`, `merge_pr`, `merge_local`), daemon and supervision control (`daemon_start/stop/restart`, `watch_start/stop`), direct repo mutation (`repo_edit/commit/push/merge`), un-gated public relay emission and linking (`public_followup_emit`, `relay_link`), fleet synchronization and inactive outcome reconciliation (`fleet_sync`, `inactive_reconcile`), remote backlog outbox receipt (`backlog_receive`), and session launch/lifecycle machinery (`session_start`, `sessionstart_run`, `sessionstart_cursor`, `herdr_lab`, `herdr_ci_cleanup`, `session_cleanup`, `claude_trust`, `agy_trust`, `claude_stop_autoarm`, `herdr_eventwait`, `herdr_workspace_move`, `backend_select`, covering the sourced `backends/*.sh` provider adapters). The readable half of that area is mirrored instead: `dispatch_resolve` prints a dispatch plan without launching anything and `sessionstart_nudge` prints at most one start line.
