...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR polling (merge or automated-reviewer submissions) or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Ahead of that ordinary stale classification, the watcher also runs a silent idle>2h staleness auto-close backstop for ordinary ship tasks: once a task's pane hash has sat unchanged for `FM_STALENESS_AUTOCLOSE_SECS`, it is not declared paused/captain-held or parked at a captain-relevant gate, and `crew_is_provably_working` (checked once per stale hash) says no, the watcher reclaims its live process via `bin/fm-teardown.sh --staleness-autoclose` rather than surfacing a wake. A human-conversation guard sits ahead of that reclaim so a captain reviewing a quiet reap-candidate pane (a non-captain-relevant idle pane such as a `working:`/`resolved:` pane whose crew looks finished) does not lose it mid-look: on a herdr backend the watcher stamps `state/.focus-<key>` on every idle poll it observes the pane focused, and blocks the reclaim while the pane is focused now or was last focused within `FM_STALENESS_FOCUS_GRACE_SECS` (default 5 min); an unreadable focus query also blocks, erring toward not reaping, but that block is bounded by `FM_STALENESS_AUTOCLOSE_MAX_RETRIES` consecutive unreadable polls (any readable poll resets the count), after which the unreadable pane is subjected to the same within-grace recency check as an unfocused one so all three focus branches converge on one reap rule (reap only when the pane is neither focused now nor focused within `FM_STALENESS_FOCUS_GRACE_SECS`), and a persistently-unreadable pane such as a dead or destroyed herdr runtime with no fresh focus marker falls through to reap while a pane focused within the grace window stays blocked even at the bound, and a non-herdr backend is a no-op that leaves the reclaim unchanged. `fm-teardown.sh` alone decides landed versus unlanded from the branch: landed work gets the ordinary full teardown, and unlanded work only has its runtime endpoint reclaimed and a triage bead filed into the `staleness` federated store (`bin/fm-staleness-file.sh`), leaving the worktree, branch, and every uncommitted change untouched for later deliberate triage. A failed reclaim retries with doubling backoff up to `FM_STALENESS_AUTOCLOSE_MAX_RETRIES` times before falling through to ordinary stale surfacing for that pane hash. This backstop also runs while away; a successful reclaim during that window appends one line to `state/.staleness-autoclose-afk.log`, which `bin/fm-afk-return.sh` surfaces as durable catch-up evidence on return.
Those actionable wakes are written to a durable local queue (`state/.wake-queue`) before detector state advances, so a missed process exit can be recovered by draining the queue.
When a canonical validated PR poll returns `merged` or `bot-review`, the watcher appends that durable notification before publishing a private receipt bound to the poll's registration, bytes, file identities, metadata, provider, URL, and task ID.
`merged` is terminal and triggers retirement; `bot-review` (GitHub only) signals a new automated-reviewer submission and keeps the poll armed for further reviews and eventual merge.
...
Ahead of that run attribution, and only under the beads backlog backend, a closed linked bead reconciles the read as a lifecycle-derived `done` - but only when the task also has no open decision and no unlanded work, so a bead that `fm-ledger.sh` drop-recovery closed without landing does not read as complete; an open bead changes nothing, so live work still resolves through the run-step and pane logic ([configuration.md](configuration.md) owns that completion-side rule under "Backlog backend").
...
Generated secondmate charters require an intake acknowledgement containing the `corr=<id>` token before doing any work on a marked request, then a terminal result line also containing `corr=<id>` after finishing; non-marked status updates reject generic receipt or start acknowledgements, key only supervisor-actionable material phase reports, and close an opened phase with a same-key later state or `resolved` event, while the structured home remains authoritative even if that closure is missing.
...
On `attached` it stays live across identity-matched successors, and a child close with no actionable reason either attaches to a verified healthy successor, exits zero with a benign `watcher: idle` line when the liveness beacon is still fresh (a clean one-shot exit whose durable wake is already enqueued), or - only on a stale, expired, or absent beacon - becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result.
...
`bin/fm-watch-cycle-lib.sh` is that ledger's only reader, and both supervision-down banners (`bin/fm-guard.sh`, `bin/fm-turnend-guard.sh`) print its classification so a harness-killed watcher is not mistaken for one that ended cleanly.
...
On a quiet close it verifies live watcher health before exiting silently, re-arming in its own foreground tree up to `FM_AUTOARM_MAX_REARMS` and escalating to a continuity-lost rewake rather than handing continuity back to itself.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns its refusal contract and the stale-lock recovery procedure, while [`bin/fm-landed-lib.sh`](../bin/fm-landed-lib.sh)'s header owns the landed-work proofs and PR-discovery fallback themselves.
That predicate is a shared library rather than teardown-private code because `bin/fm-crew-state.sh`'s closed-bead gate asks the same question and must not source a mutating script; see [configuration.md](configuration.md) under "Backlog backend".
...
Clean default-branch clones fast-forward to `origin/<default>`, and two off-default states that provably strand nothing are moved onto the default branch before the same fast-forward path runs: a clean detached HEAD holding no unique commits is re-attached, and a clean named branch that is gone from origin whose whole contribution is already contained in `origin/<default>` - including when it was squash-merged there, which no commit-by-commit ancestry test can see - is left behind without ever being deleted.
Dirty clones, branches still live on origin or still holding content of their own, detached HEADs with unique commits, diverged defaults, and default branches checked out in another worktree are reported as `STUCK:` with their behind count and left untouched.
