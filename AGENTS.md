...
- Contract index (`schema/contracts.index.json`, generated by
  `scripts/gen_contract_index.py`, gated in the CI contract-agreement job and in
  the repin engine): the single surface→script map. It must be a **union** of two
  sources, because each alone is partial — `schema/contracts.yaml` declares 64
  `bin/` commands but the decision tools point their contract at TS source, and
  `schema/matrix.md` is a human view of the depended-on subset with no
  `fm-captain-hold.sh` row at all, hiding the decision family entirely;
  `manifest/FEATURES.yaml`'s `upstream_command` is where those wrappers live.
  The doorway refuses any tool whose contract script is missing on this home with
