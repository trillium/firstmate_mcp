...
`fm_mcp_server.py` is trimmed from 24 full-coverage tools to 19 smarts-only tools with no dependency beyond Python 3.
...
`AUTH.md` documents the smarts-only tiers: open reads, reversible steers, launch-authorized writes, external sends, and the code-forbidden list.
`test_client.py` now runs 67 checks: the original 17 plus launch-tool refusal paths plus unknown-tool proof for every removed surface.
